extern crate rand;
use rand::Rng;

fn predict_linear_classification(w:Vec<f32>, xk: &Vec<f32>)-> i8{
    let mut sum = w[0];
    for i in 0..xk.len(){
        sum += w[i + 1] * xk[i];
    }
    return if sum >= 0.0 { 1 } else { -1 }
}

fn train_rosenblatt(mut w: Vec<Vec<f32>>, x: Vec<Vec<f32>>, y:Vec<i8>, nb_iter:i32, alpha :f32) {
    for it in 0..nb_iter {
       let  k = 0; // Random à mettre
        let gxk: i32 = 0;
        //gxk = predict_linear_classification(w,x[k]);
        for i in 0..x[1].len() {
            w[i + 1] += alpha * (y[k] - gxk as i8) as f32 * x[k][i];
        }
        w[0] +=   alpha * (y[k] - gxk as i8) as f32;
    }
}

fn main(){
    let w = vec![0.9, 0.8, 0.36];
    let y = vec![-1,1,1];
    let xk = vec![vec![0.2, 0.4], vec![0.73, 0.94]];
    let yk = vec![vec![0.2, 0.4], vec![0.73, 0.94]];
    let alpha : f32 = 0.1;
    let nb_it = 15;
    train_rosenblatt(xk, yk, y, *&nb_it, *&alpha);
    println!("{}",predict_linear_classification(w, &xk[0]));
}